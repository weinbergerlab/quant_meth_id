...
require(shiny)
source('./functions/WaveletPkg.R')
source('./functions/plot.wave2.R')
source('./functions/biennial.func.R')
...
-First create an index variable for time (1,2,3...t) -Then use sine and
cosine functions to generate a wave with specified period -12 month
harmonic (sine and cosine waves)
...
## Generate count data with known periodicity -120 months total (10
years of data) -Has both a 10 and 20 month cycle -Amplitude=1.5
...
##Create the needed harmonic variables with 12, 24, and 48 month
periods
...
Winner is Model 2 (12 and 24 month period). Models 2 and 3 have similar
AIC score (within 2 points), so we prefer the simpler model
...
Imaine this is RSV case data from 2 states, and we want to investigate
the epidemic characteristics in these states and the lag between states
...
```{r, fig.retina = 2}
cwt3 <- complete.cwt(wave2, dj=1/1000, dt=1/10)
plot.wave2(cwt3 )
```
...
##Convert phase difference to months
...
## Coherence analysis Evaluate coherence between the 2 series. At which
