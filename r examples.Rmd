...
```{r harmonics, echo=TRUE, fig.height = 3}
t<-1:120
sin12<-sin(2*pi*t/12)
cos12<-cos(2*pi*t/12)
...
##Vary the period (sin(2*3.14*t/period))
...
amp1=1.5
freq1=1/10 # high frequency

amp2=2.0
freq2=1/20 #low frequency
...
---
```{r plots1, echo=FALSE, fig.height = 5, fig.width = 8}
par(mfrow=c(2,2), mar=c(2,2,1,1))
plot(xt1a, bty='l' , main='Component 1', type='l' , ylim=c(-2.5, 2.5))
plot(xt1b, bty='l' , main='Component 2' , type='l', ylim=c(-2.5,2.5))
plot(xt1a+xt1b, bty='l' , main='Component 1+2' , type='l', ylim=c(-4,4))
plot(t, xt1,type="l", lwd=3, xaxt="n",bty="l",ylab="Cases of Disease", xlab="Month(n)", main="Cases")

```

##Fast Fourier Transform
-Can we use FFT to detect the correct period?
```{r fft1, echo=TRUE}
# Do a FFT
fft1=fft(xt1)
fft1[1:10] 
```
-Generates a nunch of imaginary numbers that are hard to interpret
...
```{r plot_fft}
par(mfrow=c(1,1))
  plot.frequency.spectrum(fft1, xlimits=c(2,20))
...
shinyApp(
  ...
  server = function(input, output) {
    ...
    fft.s=reactive ({fft(xt.slide())})

    output$periodPlot = renderPlot({
      par(mfrow=c(1,2), mar=c(3,1,1,1))
   plot(t, xt.slide(),type="l", lwd=3, xaxt="n",bty="l",ylab="Cases of Disease", xlab="Month(n)", main="Cases")
...
## Fit a simple poisson regression wit just 10 month period
```{r harm_reg1}
fit1a <- glm(xt1 ~ sin10+cos10, family='poisson' )
pred1a<- fitted(fit1a)
summary(fit1a)
par(mfrow=c(1,1))
...
## Add in 20 month periodicity
```{r harm_reg2}
fit2a <- glm(xt1 ~ sin10+cos10 +sin20 +cos20, family='poisson' )
pred2a<- fitted(fit2a)
summary(fit2a)
par(mfrow=c(1,1))
plot (t, xt1,bty="l", xaxt="n",yaxt="n",xlab="Time",ylab="Cases of Disease", type="p")
...
## Add in 40 month period 
```{r harm_reg3}
fit3a <- glm(xt1 ~ sin10+cos10 +sin20 +cos20 +sin40 +cos40, family='poisson' )
pred3a<- fitted(fit3a)
summary(fit3a)
par(mfrow=c(1,1))
plot (t, xt1,bty="l", xaxt="n",yaxt="n",xlab="Time",ylab="Cases of Disease", type="p")
...
Winner= Model 2 (10 and 20month period). Models 2 and 3 have similar AIC score (within 2 points), so we prefer the simpler model
```{r aic}
 AIC(fit1a)
 AIC(fit2a)
  AIC(fit3a)
...
#Wavelets
##Simulate some data with 12 and 24 month periods
#Simulate some  data that has a 12 month and 24 month period 
```{r, echo=FALSE}
  n=120 # 10 years
  t <- seq(1,n)
 amp1=5
...
#other seris shifted by 3 months
 xt2a=amp1*cos(2*3.14159*t*freq1+1)
 xt2b=amp2*cos(2*3.14159*t*freq2+1)

 #Simulate some poisson count data
 xt1=rpois(n,exp((xt1a + xt1b)))
 xt2= rpois(n,exp(xt2a+xt2b))
 plot(t, xt1,type="l", lwd=3, xaxt="n",yaxt="n",bty="l",ylab="Cases of RSV", xlab="Month(n)")
